...
| `AGENTOS_URL` | no | `http://127.0.0.1:8000` | Scheduler base URL. Set to your ALB domain in production so cron triggers reach AgentOS. |
...
# Edit infra/secrets/prd_api_secrets.yml with your API keys
...
- **Local dev**: Use `.env` file (loaded by `load_dotenv()`)
- **AWS production**: Use `infra/secrets/prd_api_secrets.yml` (uploaded to AWS Secrets Manager, pulled by ECS at runtime)

ECS task role automatically handles AWS service auth (Bedrock, etc.) — no need to put AWS credentials in secrets.
