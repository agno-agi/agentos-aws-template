...
Deploy a multi-agent system to production on AWS.
...
- [Agno CLI](https://docs.agno.com/cli) installed
...
| ECS Service & Task | Runs your AgentOS container |
| RDS Instance | PostgreSQL database with pgvector |
| Load Balancer | HTTP/HTTPS traffic routing |
| Security Groups | Network access control |
| Secrets Manager | Secure credential storage |

### Connect to control plane

1. Configure HTTPS for your load balancer ([guide](https://docs.agno.com/production/aws/domain-https))
...
```sh
ag infra down --env prd             # Stop all resources
```
...
CONTAINER_NAME=agentos-aws-template-prd-api
...
├── infra/
│   ├── dev_resources.py    # Local Docker infrastructure
...
3. Restart: `docker compose restart` (local) or redeploy to AWS
...
3. Rebuild: `docker compose up -d --build` (local) or `ag infra up --env prd` (AWS)
...
Edit `infra/prd_resources.py` to adjust:
